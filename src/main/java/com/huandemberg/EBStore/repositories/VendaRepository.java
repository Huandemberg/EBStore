...
public interface VendaRepository extends JpaRepository<Venda, Long> {
        ...
        @Query("SELECT v AS venda, SUM(v.valorCliente) OVER () AS totalDebito " +
        "FROM Venda v " +
        "WHERE v.cliente.id = :clienteId AND v.situacao = 0")
        List<ClienteDebitoProjection> findRelatorioVendasClientesDebito(@Param("clienteId") Long clienteId);
